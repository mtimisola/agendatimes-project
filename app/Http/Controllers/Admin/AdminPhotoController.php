...
class AdminPhotoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $photo->language_id = $request->language_id ?? 1; // ✅ fallback default language
        $photo->save();
    ...
    public function update(Request $request, $id) 
    {
        ...
        if ($request->hasFile('photo')) {
            ...
            if (file_exists(public_path('uploads/'.$photo_data->photo))) {
        ...
        $photo_data->language_id = $request->language_id ?? 1;
        $photo_data->update();
